    ...
    <div class="w-11/12 xl:w-1/4 md:w-1/2 mx-auto mt-2 flex flex-col md:flex-row md:flex-wrap justify-center gap-2">
        <?php if (esc($XAIaggs['twitter'])): ?>
            <div class="w-full md:w-52 mx-auto p-1 flex border border-[<?= esc($colors['acc_color']) ?>] rounded">
                ...
                <div class="self-center flex gap-1 copy_container">
                    <span class="self-center ml-2 font-semibold text_to_copy">@
                        <?= esc($XAIaggs['twitter']) ?>
                    </span>
        ...
        <?php if (esc($XAIaggs['facebook'])): ?>
            <div class="w-full md:w-52 mx-auto p-1 flex border border-[<?= esc($colors['acc_color']) ?>] rounded">
                ...
                <div class="self-center flex gap-1 copy_container copy_container">
                    <span class="self-center ml-2 font-semibold text_to_copy">@
                        <?= esc($XAIaggs['facebook']) ?>
                    </span>
                    <button class="btn_copy">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6">
        ...
        <?php if (!esc($XAIaggs['twitter']) && !esc($XAIaggs['facebook']) && !esc($XAIaggs['instagram']) && !esc($XAIaggs['linkedin']) && !esc($XAIaggs['youtube']) && !esc($XAIaggs['tiktok'])): ?>
            <h1 class="mt-2 text-center text-2xl font-semibold text-[<?= esc($colors['acc_color']); ?>]">No XAIaggs
                registred yet!</h1>
        <?php endif ?>
    </div>
</div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        var copyButtons = document.querySelectorAll(".btn_copy");

        copyButtons.forEach(function (copyButton) {
            copyButton.addEventListener("click", function () {
                var container = copyButton.closest(".copy_container");
                var textToCopy = container.querySelector(".text_to_copy");
                ...
                setTimeout(function () {
                    copyButton.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-6 h-6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M15.666 3.888A2.25 2.25 0 0013.5 2.25h-3c-1.03 0-1.9.693-2.166 1.638m7.332 0c.055.194.084.4.084.612v0a.75.75 0 01-.75.75H9a.75.75 0 01-.75-.75v0c0-.212.03-.418.084-.612m7.332 0c.646.049 1.288.11 1.927.184 1.1.128 1.907 1.077 1.907 2.185V19.5a2.25 2.25 0 01-2.25 2.25H6.75A2.25 2.25 0 014.5 19.5V6.257c0-1.108.806-2.057 1.907-2.185a48.208 48.208 0 011.927-.184" />
                        </svg>`;
